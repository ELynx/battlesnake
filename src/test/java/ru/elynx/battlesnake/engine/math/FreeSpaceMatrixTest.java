...
class FreeSpaceMatrixTest {
    ...
    void test_creation() {
        for (int w = 1; w < 50; ++w) {
            for (int h = 1; h < 50; ++h) {
                FreeSpaceMatrix tested = FreeSpaceMatrix.emptyMatrix(w, h);
                final int freeSpaceInEmptyMatrix = tested.getFreeSpace(0, 0);
    ...
    void test_get_space_outside() {
        FreeSpaceMatrix tested = FreeSpaceMatrix.emptyMatrix(11, 11);
        for (int x = -1; x < 12; ++x) {
            assertEquals(0, tested.getFreeSpace(x, -1));
            assertEquals(0, tested.getFreeSpace(x, 12));
        }
        for (int y = -1; y < 12; ++y) {
            assertEquals(0, tested.getFreeSpace(-1, y));
            assertEquals(0, tested.getFreeSpace(12, y));
        }
    }
    ...
    FreeSpaceMatrix buildFromAscii(String mazeAsciiGameState) {
        GameStateDto tmp = new AsciiToGameState(mazeAsciiGameState).setStartSnakeSize(1).build();
